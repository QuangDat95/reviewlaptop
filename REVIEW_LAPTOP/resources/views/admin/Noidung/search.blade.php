                ...
                <div>
                    <div class="panel-body">
                        <p class="text-muted" style="padding-top:50px"><a class="btn btn-primary"
                                href="{{ route('noidung.create') }}">Thêm nội dung</a></p>
                                <p>Tìm thấy {!! count($noidungs) !!} danh mục với từ khóa "<?php echo $key?>"</p>
                        <div class="mt-40">
                            ...
                            <div>
                                <table class="table mb-0">
                                    <thead class="thead-dark" style="padding-right:50px">
                                        <tr>
                                            <th>#</th>
                                            <th>Tiêu đề</th>
                                            <th>Nội dung ngắn</th>
                                    ...
                                    <tbody>
                                        @if(count($noidungs) == 0)
                                        <tr>
                                            <td colspan="3">Không có nội dung nào</td>
                                        </tr>
                                        @else
                                        @foreach($noidungs as $key => $noidung)
                                        <tr>
                                            <td>{{$noidung->id}}</td>
                                            <td>{{$noidung->Tieu_de}}</td>
                                            <td>{{$noidung->ND_ngan}}</td>
                                            <td>{{$noidung->maytinh->Ten_may}}</td>
                                            <td>
                                                <form action="{{route('noidung.destroy', $noidung->id)}}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('noidung.edit', $noidung->id)}}"
