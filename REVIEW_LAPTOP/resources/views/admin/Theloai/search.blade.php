    ...
    <div class="wrapper  theme-3-active pimary-color-blue">
        ...
        <nav class="navbar navbar-inverse navbar-fixed-top">
            <div class="mobile-only-brand pull-left">
                ...
                <form action="{{route('theloaisearch')}}" method="GET" id="search_form" role="search" class="top-nav-search collapse pull-left">
            ...
            <div class="container">
                ...
                <div class="container">
                    <div class="panel-body" style="padding-top:50px">
                        <p class="text-muted" style="padding-top:10px"><a class="btn btn-primary" href="{{ route('theloai.create') }}">Thêm hãng</a></p>
                        <p>Tìm thấy {!! count($Alltype) !!} danh mục với từ khóa "<?php echo $key?>"</p>
                        <div class="mt-40">
                            ...
                            <div>
                                <table class="table mb-0">
                                    ...
                                    @if(count($Alltype) == 0)
                                        ...
                                        @else
                                        @foreach($Alltype as $key => $alltype)
                                        <tr>
                                            <td>{{$alltype->id}}</td>
                                            <td>{{$alltype->Ten_hang}}</td>
                                            <td>    
                                                <form action="{{route('theloai.destroy', $alltype->id)}}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('theloai.edit', $alltype->id)}}" class="btn btn-primary">Sửa</a>
                                                    <input type="submit" class="btn btn-danger" value="Xóa"  onClick="return confirm('Bạn có muốn xóa hãng này?');">
                                                </form>
                                            </td>
                                        </tr>
                                        @endforeach
